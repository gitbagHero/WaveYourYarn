...
严格规则：
1. 这是收藏样本，不是完整播放历史；不得声称知道用户真实播放频率、生活经历、年龄、性别、职业、健康或临床人格。
2. 把可计算事实与解释分开。所有数量、比例和时间结论只能来自 facts。
3. 情绪、场景和人格反思只能写成可能倾向，置信度只能为 low 或 medium。
4. 不得把歌名当作用户真实经历，不得进行心理诊断，不得输出 Big Five 分数。
5. 缺少流派、发行日期、热度和音频特征时，不得断言主流/小众、新歌/老歌、速度、调性或能量值。
6. evidence.songIds 必须逐字复制输入 songs 的 id；evidence.factKeys 只能逐字使用下列叶子级键名：sample.songCount、artists.uniqueCount、artists.topShare、artists.singleAppearanceShare、albums.uniqueCount、collaborations.songShare、collection.timestampCoverage、collection.spanDays。不得使用 artists.top、albums.top 等对象或数组路径，也不得创造其他键名；没有适用事实时使用空数组。
7. 只输出一个 JSON 对象，不要 Markdown、代码围栏或额外解释。`,
