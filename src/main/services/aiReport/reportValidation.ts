...
export function parseAndValidateAIReport(
  ...
  const report: AIReportContentV1 = {
    ...
    tasteSnapshot: readTasteSnapshot(parsed.tasteSnapshot),
...
function parseJsonObject(rawContent: string): Record<string, unknown> {
  ...
  try {
    ...
    return readObject(parsed, 'report')
  } catch (error) {
...
function readTasteSnapshot(value: unknown): AIReportContentV1['tasteSnapshot'] {
  const object = readObject(value, 'tasteSnapshot')
  return {
...
function readInsights(
  ...
  return readArray(value, path, minimum, maximum).map((entry, index) => {
    ...
    const object = readObject(entry, itemPath)
    return {
